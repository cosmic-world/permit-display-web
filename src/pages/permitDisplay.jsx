...
export default function PermitDisplay() {
  ...
  const { selectedTerminal, PermitList } = useSelector(
    (state) => state.itas
  );
  ...
  useEffect(() => {
    setstartIndex((prevState) =>
        prevState + step < PermitList.length
          ? prevState + step
          : 0)
  }, [clock]);
...
function formatTime(date1) {
  ...
  useEffect(() => {
    const fetchSheetData = async () => {
      try {
        ...
        const filteredData = formattedData.filter(ele=>ele['Location_Name'].toLowerCase() === locationName.toLowerCase()).filter(ele=>formatTime(ele['Clearance given till'])>new Date().toLocaleTimeString('en-GB'));     
  ...
  return (
        <div
          className={"d-flex justify-content-center align-items-center w-100 h-100 p-2"}
          style={{ overflow: "none", overflowX: "auto", backgroundColor:"#dee4ea"}}
        >
                  ...
                  return (
                    <tr key={i}>
                      <td style={{ textAlign: "center" }}>
                        {permit? formatDate(permit.Timestamp) : ''}
                      </td>
                      <td style={{ textAlign: "center" }}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        {permit? permit['Receiver Name (IOC OFFICER NAME)'] : ''}
                      </td>
                      <td style={{ textAlign: "center" }}>
                        {permit? formatTime(permit['Clearance given from']) : ''}
                      </td>
                      <td style={{ textAlign: "center" }}>
                        {permit? formatTime(permit['Clearance given till']) : ''}
                      </td>
                      <td style={{ textAlign: "center" }}>
